...
for sample in 'totouples_DYToMuMu_Summer12_February2014_noskim_FSR' 'DYToMuMu_Summer12_PU_S10_noskim_FSR'  
do
		...
		if [ "$sample" = "totouples_TTJets_SemiLeptMGDecays_Summer12_November2013" ] || [ "$sample" = "totouples_TTJets_FullLeptMGDecays_Summer12_November2013_v2" ]
                then
                        pileup_set=RD1
                        lumi_set=2012_22Jan
                        isZgamma=3
		...
		do
				...
				##qsub batchJob_3.sh ${sample} ${sample}_${isZgamma}_injRe${inj_resolution}_v10_test ${isZgamma} ${lumi_set} ${pileup_set} ${lowMuMuCut} ${highMuMuCut} ${scCorrection} 1.0 3 0.0 ${inj_resolution} ${itoy} -l os=sl6 
				hadd miniTree_${sample}_${isZgamma}_injRe${inj_resolution}_v10_test_partALL.root miniTree_${sample}_${isZgamma}_injRe${inj_resolution}_v10_test_part[0-9]*root
				##mv miniTree_${sample}_${isZgamma}_injRe${inj_resolution}_v10_test_part[0-9]*root stored_miniTree/
                        	##mv ${sample}_${isZgamma}_injRe${inj_resolution}_v10_test_part[0-9]*err stored_errput/
                        	##mv ${sample}_${isZgamma}_injRe${inj_resolution}_v10_test_part[0-9]*out stored_output/
		done
