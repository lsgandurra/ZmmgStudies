  ...
  runMin_map(), rgen_(NULL)
{
  ...
  rgen_=new TRandom3(0); // inizializzo il generatore con seed impostato sull'ora //FIXME 
  return;
}
...
TTree *EnergyScaleCorrection_class::GetCorrTree(TChain *tree, bool fastLoop, 
  ...
  newTree->Branch("corrEle", scaleEle_, "corrEle[2]/F");
  ...
  for(Long64_t ientry = 0; ientry<tree->GetEntriesFast(); ientry++){
    ...
    scaleEle_[0] = getScaleOffset(runNumber_, fabs(etaSCEle_[0]) < 1.4442, 
				 R9Ele_[0],etaEle_[0]);
    scaleEle_[1] = getScaleOffset(runNumber_, fabs(etaSCEle_[1]) < 1.4442, 
				 R9Ele_[1],etaEle_[1]);
    newTree->Fill();
  }
