...
*/
       ...
       //*******************************FAN
   /* 
        ofstream outfile(Form("triggerTest_%s_part%i_%i.txt", sample_char, ijob, isZgammaMC));
        ofstream outfile1(Form("triggerTestResult_%s_part%i_%i.txt", sample_char, ijob, isZgammaMC));
        vector<int> Trigger_EveTotal;
        vector<string> Trigger_Nom;
        int triggerNum = 0;
*/
		...
		isAfterFSRCut8 = 1;
             ...
             /* 
              ...
              for(int i_mmg = 0; i_mmg < nbMuMuGammaAfterID[6] ; i_mmg++){
                        TRootPhoton* myphoton;
                        myphoton = (TRootPhoton*) photons->At(MuMuGammaCandidates[6][i_mmg].first);
                        if( Photon_scale[MuMuGammaCandidates[6][i_mmg].first]*(myphoton->Et()) > 25 ){
                        checkPt = 1;
                      }
                 }
              ...
              if(checkPt == 1){
                ...
                vector<string> hltnames = event->hltAcceptNames();
                const unsigned int numTrigger = hltnames.size();
                outfile << "Run:  " << event->runId() << "       Event:  " << event->eventId() << ":   " << endl;
                for (unsigned int itri = 0; itri < numTrigger; itri++){
                    if(hltnames.at(itri).find("HLTriggerFinalPath") == 0) continue;
                    outfile << hltnames.at(itri) << endl;
                    int itest = 0;
                    for(int itotal = 0; itotal < triggerNum; itotal++){
                        //if(Trigger_Nom.at(itotal) == hltnames.at(itri))  itest==1;
                        if(Trigger_Nom[itotal].compare(hltnames.at(itri)) == 0){
                             itest = 1;
                             Trigger_EveTotal[itotal] = Trigger_EveTotal[itotal] + 1;
                             //cout << itotal << " : " << Trigger_EveTotal[itotal] << " : " << Trigger_EveTotal.size() << endl;
                         }
                    }
                    if(itest == 0){
                       triggerNum++;
                       Trigger_Nom.resize(triggerNum);
                       Trigger_Nom[triggerNum-1] = hltnames.at(itri);
                       Trigger_EveTotal.resize(triggerNum);
                       Trigger_EveTotal[triggerNum-1] = 0;
                       Trigger_EveTotal[triggerNum-1] = Trigger_EveTotal[triggerNum-1] + 1;
                       //cout << "I am  " << triggerNum-1 << " : " << Trigger_EveTotal[triggerNum-1] << " : " << Trigger_EveTotal.size() << endl;
                       //outfile1 << Trigger_Nom.at(triggerNum-1) << endl;          
                    }
                }
                outfile << endl;
...
//				FillMMG(myphoton, mymuon1, mymuon2, Photon_scale[MuMuGammaCandidates[8][i_mmg].first], doMC, doPhotonConversionMC, doR9Rescaling, mcParticles, reader);
		...
		*/
        ...
        //*********************************************FAN
            /*for(int nfinal = 0; nfinal < Trigger_Nom.size(); nfinal++){
                  outfile1 << Trigger_Nom[nfinal] << " : " << Trigger_EveTotal[nfinal] << endl;
            }
*/
       //********************************************FAN
