...
do
		...
		for itoy in `seq 1 100`
                do

				echo "sample = $sample, pileup_set = ${pileup_set}, lumi_set = ${lumi_set}, isZgamma = ${isZgamma}, inj_resolution = ${inj_resolution}, itoy = ${itoy}"
				
				qsub batchJob_temp.sh DYToMuMu_Summer12_NewMuonID DYToMuMu_Summer12_NewMuonID_NewSelection_2_MuSys_itoy80_v6_RecoEnergy 2 PU_S10 35 9999 1.0 1.0 3 0.0 0.0 80
				qsub batchJob.sh ${sample} ${sample}_NewSelection_${isZgamma}_MuSys_itoy${itoy}_v6_RecoEnergy ${isZgamma} ${lumi_set} ${pileup_set} ${lowMuMuCut} ${highMuMuCut} ${scCorrection} 1.0 3 0.0 ${inj_resolution} ${itoy} 
				##hadd miniTree_${sample}_NewSelection_${isZgamma}_MuSys_itoy${itoy}_v6_RecoEnergy_partALL.root miniTree_${sample}_NewSelection_${isZgamma}_MuSys_itoy${itoy}_v6_RecoEnergy_part[0-9]*root
                        	##echo "miniTree_${sample}_${Zgamma2}_${SetOfCorrections2}_Triangle_v1_part[0-9].root"
				##mv miniTree_${sample}_NewSelection_${isZgamma}_MuSys_itoy${itoy}_v6_RecoEnergy_part[0-9]*root stored_miniTree/
                        	##mv ${sample}_NewSelection_${isZgamma}_MuSys_itoy${itoy}_v6_RecoEnergy_part[0-9]*err stored_errput/
                        	##mv ${sample}_NewSelection_${isZgamma}_MuSys_itoy${itoy}_v6_RecoEnergy_part[0-9]*out stored_output/
		done
