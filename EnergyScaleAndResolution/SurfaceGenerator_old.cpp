	...
	if(option == "MmumuFit")
	{
		...
		for(int i = 0; i < nbBins; i++)
		{
			for(int j = 0; j < nbBins; j++)
				...
				if(reducedChain->GetEntries(cut) > 10) nbFits++;
				if(nbFits == nbFitsMax || (i == nbBins-1 && j == nbBins-1))
				{
					...
					//cout<<endl<<"reducedChain->GetEntries(cut) = "<<reducedChain->GetEntries(cut)<<endl;
					//cout<<endl<<Form("qsub batchJob_surface.sh %s %s %d %s %d %d %d %d %d %s",directoryName.c_str(),dataType.c_str(),surfaceBinning,fitFunction.c_str(), i_begin, i_end, j_begin, j_end, iterSub, MmumuOption.c_str())<<endl;
					chainSub = Form("qsub batchJob_surface.sh %s %s %d %s %d %d %d %d %d %s",directoryName.c_str(),dataType.c_str(),surfaceBinning,fitFunction.c_str(), i_begin, i_end, j_begin, j_end, iterSub, MmumuOption.c_str()) ;			
