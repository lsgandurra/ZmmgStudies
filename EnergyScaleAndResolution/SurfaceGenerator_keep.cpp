		...
		for(int i = 0; i < nbBins; i++)
		{
			for(int j = 0; j < nbBins; j++)
				...
				if(nbFits == nbFitsMax || (i == nbBins-1 && j == nbBins-1))
				{
					...
					chainSub = Form("batchJob_surface.sh %s %s %d %s %d %d %d %d %d %s binsToFit_%s_%d.txt",directoryName.c_str(),dataType.c_str(),surfaceBinning,fitFunction.c_str(), i_begin, i_end, j_begin, j_end, iterSub, MmumuOption.c_str(),dataType.c_str(),iterSub) ;
					...
					//system(chainSub.c_str());
				
					chainSub = "batchJob_surface.sh";
					string chainSub2 = Form("%s",directoryName.c_str());
					string chainSub3 = Form("%s",dataType.c_str());
					string chainSub4 = Form("%d",surfaceBinning);
					string chainSub5 = Form("%s",fitFunction.c_str());
					string chainSub6 = Form("%d",i_begin);
					string chainSub7 = Form("%d",i_end);
					string chainSub8 = Form("%d",j_begin);
					string chainSub9 = Form("%d",j_end);
					string chainSub10 = Form("%d",iterSub);
					string chainSub11 = Form("%s",MmumuOption.c_str());	
					string chainSub12 = Form("binsToFit_%s_%d.txt",dataType.c_str(),iterSub);
					
	
   					execlp("/opt/sge/bin/lx-amd64/qsub", "qsub" ,chainSub.c_str(),chainSub2.c_str(),chainSub3.c_str(),chainSub4.c_str(),chainSub5.c_str(),chainSub6.c_str(),chainSub7.c_str(),chainSub8.c_str(),chainSub9.c_str(),chainSub10.c_str(),chainSub11.c_str(),chainSub12.c_str(), NULL);
