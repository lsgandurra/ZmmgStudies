#./bin/bash
directory=Results_thesis_v1_7bins
		...
		do
			...
			do
				...
				do
					...
					then 
						directory=Results_2011_thesis_v1_7bins			
					fi
					
					if [ "$analysisVersion" = "2012" ]
                                        then 
                                                directory=Results_2012_thesis_v1_7bins        
                                        fi	
			
					echo "dataType = ${dataType}, fitVariable = ${fitVariable}, fitPercentage = ${fitPercentage}" 	
					qsub batchJob.sh input ${directory}  ${dataType} ${fitVariable} Photon_Et LimitesAllPt7Bin.txt ${fitPercentage} ${injectedResolution} ${analysisVersion} -l os=sl6
				done
			done 
		done
        done
