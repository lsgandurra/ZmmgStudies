...
int main(int argc, char *argv[])
{
	...
	for(int iarg = 0 ; iarg < argc; iarg++)
        ...
        if( argc == 1 ) 
        {
                cerr << "arguments should be passed : directoryName, dataType, surfaceBinning, fitFunction, i_begin, i_end, j_begin, j_end, iterSub, MmumuOption" <<endl; 
                return 1;
...
/*
	if(MmumuOption == "MmumuRECO" || MmumuOption == "MmumuGEN") //Add HLTnames in the mumu miniTree
        {   
        	cut = "isMM_nonFSR == 1";
	...
	cout<<endl<<"cut = "<<cut;
	reducedChain_temp = (TChain *) chain->CopyTree(cut);
*/
	for(int i = i_begin; i <= i_end; i++)
	{
		...
		{
			cout <<endl<< "j = "<<j;
			if(i == i_begin && iterLoop == 0) j = j_begin; //FIXME
			iterLoop++;
	...
	cout<<endl<<"directoryNameBeforeOfstream = "<<directoryName<<endl;

	ofstream mumuFile(Form("%sMmumu_%d.txt",directoryName.c_str(),iterSub));

	cout<<endl<<"mumuVector.size() = "<<mumuVector.size()<<endl;
	for(int i = 0; i < mumuVector.size(); i++)
	{
