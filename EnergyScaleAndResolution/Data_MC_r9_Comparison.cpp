...
int main(int argc, char *argv[])
{
	...
	string directoryName = "data_MC_r9_Comparison";
	string eta = "Barrel";
	string r9 = "all";
	int log = 1;		
	...
	string fileName = "data_MC_r9_Comparison";
	...
	cout<<endl<<"dataChain->GetEntries() = "<<dataChain->GetEntries(cut)<<endl;

	TCanvas *c1 = new TCanvas("c1", "c1",0,0,600,600);
	if(log == 1) c1->SetLogy();

	TH1D *data = new TH1D("data","data", 50, 0, 1);	
	TH1D *dYToMuMuFSR = new TH1D("dYToMuMuFSR","dYToMuMuFSR", 50, 0, 1);
	TH1D *dYToMuMuNonFSR = new TH1D("dYToMuMuNonFSR","dYToMuMuNonFSR", 50, 0, 1);
	TH1D *ttJets = new TH1D("ttJets","ttJets", 50, 0, 1);
	TH1D *wJets = new TH1D("wJets","wJets", 50, 0, 1);				
		
	dataChain->Draw("Photon_r9>>data",cut);
	dYToMuMuFSRChain->Draw("Photon_r9>>dYToMuMuFSR",cut);
	dYToMuMuNonFSRChain->Draw("Photon_r9>>dYToMuMuNonFSR",cut);
	ttJetsChain->Draw("Photon_r9>>ttJets",cut);
	wJetsChain->Draw("Photon_r9>>wJets",cut);	
	...
	double nMC = dYToMuMuFSRChain->GetEntries(cut) * lumidata / lumiDY + dYToMuMuNonFSRChain->GetEntries(cut) * lumidata / lumiDY + ttJetsChain->GetEntries(cut) * lumidata / lumiTtJets + wJetsChain->GetEntries(cut) * lumidata / lumiwJets;	
	...
	cout<<endl<<endl<<"nData = "<<nData<<", nMC = "<<nMC<<", r = "<<nData/nMC<<endl<<endl;
	...
	*/
	...
	dYToMuMuFSR->GetXaxis()->SetTitle("r9");
	dYToMuMuFSR->GetYaxis()->SetTitle("Events / 5 GeV");
	/*
	...
	*/
	...
	TLegend leg(0.27,0.7,0.51,0.9,"","brNDC");
	leg.SetTextFont(42);
	...
	std::ostringstream cutString2;
        cutString2 << setprecision (2) << fixed << nMC;
